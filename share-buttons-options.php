		...
		<div style="margin-right: 160px;">
				...
				<fieldset class="fieldset_sort">
					...
					<div class="body_sort">
						...
						<?php
							...
							$style_social_name = array('Facebook', 'Google Buzz', 'Google Plus', 'LiveJournal', 'Mail.ru', 'Twitter', 'Одноклассники', 'Вконтакте','Yandex');
							$duplic_valsort = $valsort;
							array_multisort($duplic_valsort, $style_social_name);

							for($j=0;$j<count($btnsort);$j++) {
								$temp[$keysort[$j]]=$valsort[$j];
								asort($temp);
								echo '<div title="'.$valsort[$j].'" id="'.$valsort[$j].'">';
									echo '<li>'.$style_social_name[$j].'</li>';
								echo '</div>';
								$order[] = $valsort[$j];
							}
						?>
						...
						<?php
							...
							if(isset($_POST['do_submit']))  {
								...
								$query = "UPDATE $wpdb->options SET option_value = '$ids' WHERE option_name = 'buttons_sort'";
				...
				<fieldset class="fieldset_style">
					...
					<div id="customize_type" class="body_style">
						<div style="width:450px;">
							<div>
								<input type="radio" name="opt_customize_type" id="opt_customize_type" value="soft_round" <?php echo (get_option('opt_customize_type') == 'soft_round' ? 'checked' : ''); ?> />01. <?php _e('Soft Round', $this->plugin_domain);?>
							</div>
						...
						<div style="width:450px;margin-top:15px;">
							<div>
								<input type="radio" name="opt_customize_type" id="opt_customize_type" value="soft_rect" <?php echo (get_option('opt_customize_type') == 'soft_rect' ? 'checked' : ''); ?> />02. <?php _e('Soft Rectangle', $this->plugin_domain);?>
							</div>
							<div style="margin-top:5px;margin-left:15px;">
						...
						<div style="width:450px; margin-top:15px;">
							<div>
								<input type="radio" name="opt_customize_type" id="opt_customize_type" value="classic" <?php echo (get_option('opt_customize_type') == 'classic' ? 'checked' : ''); ?> />03. <?php _e('Classic', $this->plugin_domain);?>
							</div>
							<div style="margin-top:5px;margin-left:15px;">
						...
						<div style="width:450px; margin-top:15px;">
							<div>
								<input type="radio" name="opt_customize_type" id="opt_customize_type" value="mini" <?php echo (get_option('opt_customize_type') == 'mini' ? 'checked' : ''); ?> />04. <?php _e('Classic mini', $this->plugin_domain);?>
							</div>
							<div style="margin-top:5px;margin-left:15px;">
					...
					<div class="body_enable">
						...
						<div style="float:left; width:150px;">
							<label for="facebook_button_show">
								<input name="buttons_show[]" type="checkbox" id="buttons_show" value="facebook" <?php checked(TRUE, in_array('facebook',$show)); ?> />
								<?php _e('Facebook', $this->plugin_domain) ?>
							</label>
							<br/>
							<label for="googlebuzz_button_show">
								<input name="buttons_show[]" type="checkbox" id="buttons_show[]" value="googlebuzz" <?php checked(TRUE, in_array('googlebuzz',$show)); ?> />
								<?php _e('Google Buzz', $this->plugin_domain) ?>
							</label>
							<br/>
							<label for="googleplus_button_show">
								<input name="buttons_show[]" type="checkbox" id="buttons_show[]" value="googleplus" <?php checked(TRUE, in_array('googleplus',$show)); ?> />
								<?php _e('Google Plus', $this->plugin_domain) ?>
							</label>
							<br/>
							<label for="livejournal_button_show">
								<input name="buttons_show[]" type="checkbox" id="buttons_show[]" value="livejournal" <?php checked(TRUE, in_array('livejournal',$show)); ?> />
								<?php _e('LiveJournal', $this->plugin_domain) ?>
							</label>
							<br/>
							<label for="mailru_button_show">
								<input name="buttons_show[]" type="checkbox" id="buttons_show" value="mailru" <?php checked(TRUE, in_array('mailru',$show)); ?> />
						...
						<div style="float:left;">
							<label for="odnoklassniki_button_show">
								<input name="buttons_show[]" type="checkbox" id="buttons_show[]" value="odnoklassniki" <?php checked(TRUE, in_array('odnoklassniki',$show)); ?> />
								<?php _e('Odnoklassniki', $this->plugin_domain) ?>
							</label>
							<br/>
							<label for="twitter_button_show">
								<input name="buttons_show[]" type="checkbox" id="buttons_show" value="twitter" <?php checked(TRUE, in_array('twitter',$show)); ?> />
								<?php _e('Twitter', $this->plugin_domain) ?>
							</label>
							<br/>
							<label for="vkontakte_button_show">
								<input name="buttons_show[]" type="checkbox" id="buttons_show[]" value="vkontakte" <?php checked(TRUE, in_array('vkontakte',$show)); ?> />
								<?php _e('Vkontakte', $this->plugin_domain) ?>
							</label>
							<br/>
							<label for="yandex_button_show">
								<input name="buttons_show[]" type="checkbox" id="buttons_show[]" value="yandex" <?php checked(TRUE, in_array('yandex',$show)); ?> />
			...
			<fieldset class="fieldset_position">
				...
				<div class="body_other">
					...
					<div><input type="text" name="margin_top" size="35" value="<?php echo esc_attr($this->margin_top);?>" class="regular-text" /></div>
					<br />
					<div><?php _e('Margin bottom', $this->plugin_domain);?>:</div>
					<div><input type="text" name="margin_bottom" size="35" value="<?php echo esc_attr($this->margin_bottom);?>" class="regular-text" /></div>
				</div>
				<br />
			</fieldset>
...
<!-- Vkontakte.ru Like Button -->
				...
				<div class="body_social" id="vkontakte_like_sample_buttons">
					<div><img name="vk_pic_like" id="vk_pic_like" src="<?php echo $this->plugin_url;?>/like/images/vkontakte/full-like.png" /></div>
					<br />
					<div>
				...
				<div class="body_social" id="facebook_like_button">
					<div><img name="fb_pic_like" id="fb_pic_like" src="<?php echo $this->plugin_url;?>/like/images/facebook/standart-like-light.png" /></div>
					<br />
