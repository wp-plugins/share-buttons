...
<?php	
	...
	if($imgUploaded){
        echo 'Success!<br /><img src="'.$upload_image.'" border="0" />';
	}else{
		echo 'Error(s) Found: ';
	...
	@$source_image = basename($upload_image);
	$dh = opendir('../uploads/');
	$dir = '../uploads/';

	while($fname = readdir($dh)){
		if( $fname != "." && $fname != ".." && $fname!="logo.png" && $fname!=$source_image ) {
			$files[] = $fname;
			@unlink($dir.$fname);
		}
